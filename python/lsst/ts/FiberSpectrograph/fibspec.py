...
class AVS:
  def __init__(self):
    ...
    #DLL_INT AVS_GetList(unsigned int a_ListSize, unsigned int* a_pRequiredSize,AvsIdentityType* a_pList);
    self.lib.AVS_GetList.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(AvsIdentityType)]
    self.lib.AVS_GetList.restype = ctypes.c_int

    #DLL_INT AVS_GetParameter( AvsHandle a_hDevice,unsigned int a_Size, unsigned int* a_pRequiredSize,DeviceConfigType* a_pDeviceParm);
    self.lib.AVS_GetParameter.argtypes = [ctypes.c_int, ctypes.c_uint, ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(DeviceConfigType)]
    ...
    self.lib.AVS_GetScopeData.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(ctypes.c_double)]
    ...
    # DLL_INT AVS_SetParameter( AvsHandle a_hDevice, DeviceConfigType* a_pDeviceParm);
  ...
  def prepareMeasure(self, handle, measConfig):
    ...
    # result = self.lib.AVS_PrepareMeasure(handle, ctypes.byref(measConfig))
    return result, measConfig

  def getScopeData(self, handle, numberOfPixels = None):
    # DLL_INT AVS_GetScopeData( AvsHandle a_hDevice, unsigned int* a_pTimeLabel, double* a_pSpectrum );
    if numberOfPixels == None:
